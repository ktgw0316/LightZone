...
final class LCColorConvertOpImage extends PointOpImage {
    ...
    public LCColorConvertOpImage(RenderedImage source,
        ...
        if (srcColorSpace instanceof ColorSpaceImageN &&
                dstColorSpace instanceof ColorSpaceImageN) {
    ...
    protected void computeRect(Raster[] sources,
        ...
        switch (caseNumber) {
            ...
            case 1:
                tempRas = computeRectColorSpaceImageNToRGB(sources[0], srcParam,
                        null, tempParam);
                computeRectColorSpaceImageNFromRGB(tempRas, tempParam,
                        dest, dstParam);
                break;
            ...
            case 2:
                tempRas = computeRectColorSpaceImageNToRGB(sources[0], srcParam,
                        null, tempParam);
                computeRectNonColorSpaceImageN(tempRas, tempParam,
                        dest, dstParam, destRect);
                break;
            case 3:
                computeRectColorSpaceImageNToRGB(sources[0], srcParam,
                        dest, dstParam);
                break;
            ...
            case 4:
                ...
                computeRectNonColorSpaceImageN(sources[0], srcParam,
                        tempRas, tempParam, destRect);
                computeRectColorSpaceImageNFromRGB(tempRas, tempParam,
                        dest, dstParam);
                break;
            case 5:
                computeRectColorSpaceImageNFromRGB(sources[0], srcParam,
                        dest, dstParam);
                break;
    ...
    private WritableRaster computeRectColorSpaceImageNToRGB(Raster src,
							 ImageParameters srcParam,
							 WritableRaster dest,
							 ImageParameters dstParam) {
	src = convertRasterToUnsigned(src);

	ColorSpaceImageN colorSpaceImageN
	    = (ColorSpaceImageN) srcParam.getColorModel().getColorSpace();
	dest = colorSpaceImageN.toRGB(src, srcParam.getComponentSize(), dest,
    ...
    // 2. Convert from RGB.
    // 3. Shift back to [MIN, MAX]
    private WritableRaster computeRectColorSpaceImageNFromRGB(Raster src,
							   ImageParameters srcParam,
							   WritableRaster dest,
							   ImageParameters dstParam){
	src = convertRasterToUnsigned(src);
	ColorSpaceImageN colorSpaceImageN
        ...
        dest = colorSpaceImageN.fromRGB(src, srcParam.getComponentSize(), dest,
				     dstParam.getComponentSize());
    ...
    private void computeRectNonColorSpaceImageN(Raster src,
					     ImageParameters srcParam,
					     WritableRaster dest,
					     ImageParameters dstParam,
					     Rectangle destRect) {
