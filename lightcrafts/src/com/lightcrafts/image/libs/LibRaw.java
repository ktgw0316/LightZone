...
import com.lightcrafts.image.metadata.IM_Util;
...
/* reserved */
	...
	public String getCameraMake(boolean includeModel) {
        ...
        if ( !includeModel )
            return make;
        return IM_Util.undupMakeModel( make, model.toUpperCase() );
	}
    ...
    public synchronized RenderedImage getThumbnail(int maxSize) throws UnknownImageTypeException, BadImageFileException {
		...
		if (tformat == 1) {
        	...
        	return JPEGImageType.getImageFromBuffer(thumbnail, 0, thumbnail.length, JAIContext.sRGBColorSpace, maxSize);
        } else {
        	final ColorModel cm;
        	if (tcolors == 3)
	...
	public static void main(String args[]) {
		...
		try {
			RenderedImage thumbnail = libRaw.getThumbnail(-1);
