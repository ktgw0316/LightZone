...
extern "C" JNIEXPORT jlong JNICALL Java_com_lightcrafts_utils_Lensfun_init(
    JNIEnv *env, jobject obj, jstring pathStr) {
  const auto path = env->GetStringUTFChars(pathStr, NULL);
  auto lf = std::make_unique<LC_lensfun>(path);
  return reinterpret_cast<uintptr_t>(lf.release());
}

extern "C" JNIEXPORT void JNICALL Java_com_lightcrafts_utils_Lensfun_destroy(
  ...
  auto lf = std::unique_ptr<LC_lensfun>(reinterpret_cast<LC_lensfun*>(handle));
  lf.reset();
}

extern "C" JNIEXPORT jobjectArray JNICALL
Java_com_lightcrafts_utils_Lensfun_getCameraNames(JNIEnv *env, jobject obj,
                                                  jlong handle) {
  auto lf = std::unique_ptr<LC_lensfun>(reinterpret_cast<LC_lensfun*>(handle));
...
extern "C" JNIEXPORT jobjectArray JNICALL
Java_com_lightcrafts_utils_Lensfun_getLensNames(JNIEnv *env, jobject obj,
                                                jlong handle) {
  auto lf = std::unique_ptr<LC_lensfun>(reinterpret_cast<LC_lensfun*>(handle));
...
Java_com_lightcrafts_utils_Lensfun_getLensNamesForCamera(
    JNIEnv *env, jobject obj, jlong handle, jstring cameraMakerStr,
    jstring cameraModelStr) {
  auto lf = std::unique_ptr<LC_lensfun>(reinterpret_cast<LC_lensfun*>(handle));
...
Java_com_lightcrafts_utils_Lensfun_initModifier(
    JNIEnv *env, jobject obj, jlong handle, jint fullWidth, jint fullHeight,
    jstring cameraMakerStr, jstring cameraModelStr, jstring lensMakerStr,
    jstring lensModelStr, jfloat focal, jfloat aperture) {
  auto lf = std::unique_ptr<LC_lensfun>(reinterpret_cast<LC_lensfun*>(handle));
  ...
  lf->initModifier(fullWidth, fullHeight, cameraMaker, cameraModel, lensMaker,
...
Java_com_lightcrafts_utils_Lensfun_initModifierWithPoly5Lens(
    JNIEnv *env, jobject obj, jlong handle, jint fullWidth, jint fullHeight,
    jfloat k1, jfloat k2, jfloat kr, jfloat kb, jfloat focal, jfloat aperture) {
  auto lf = std::unique_ptr<LC_lensfun>(reinterpret_cast<LC_lensfun*>(handle));
  ...
  lfLensCalibDistortion dc = {
      LF_DIST_MODEL_POLY5, focal, focal, 0, {k1, k2}, attr};
...
Java_com_lightcrafts_utils_Lensfun_distortionColor(
    JNIEnv *env, jobject obj, jlong handle, jshortArray jsrcData,
    jshortArray jdstData, jint srcRectX, jint srcRectY, jint srcRectWidth,
    jint srcRectHeight, jint dstRectX, jint dstRectY, jint dstRectWidth,
    jint dstRectHeight, jint srcPixelStride, jint dstPixelStride,
    jint srcROffset, jint srcGOffset, jint srcBOffset, jint dstROffset,
    jint dstGOffset, jint dstBOffset, jint srcLineStride, jint dstLineStride) {
  auto lf = std::unique_ptr<LC_lensfun>(reinterpret_cast<LC_lensfun*>(handle));
  ...
  lf->applyModifier(srcData, dstData, srcRectX, srcRectY, srcRectWidth,
                    srcRectHeight, dstRectX, dstRectY, dstRectWidth,
                    dstRectHeight, srcPixelStride, dstPixelStride, srcROffset,
                    srcGOffset, srcBOffset, dstROffset, dstGOffset, dstBOffset,
...
Java_com_lightcrafts_utils_Lensfun_backwardMapRect(JNIEnv *env, jobject obj,
                                                   jlong handle, jint dstRectX,
                                                   jint dstRectY,
                                                   jint dstRectWidth,
                                                   jint dstRectHeight) {
  auto lf = std::unique_ptr<LC_lensfun>(reinterpret_cast<LC_lensfun*>(handle));
  ...
  lf->backwardMapRect(reinterpret_cast<int *>(srcRect), dstRectX, dstRectY,
...
const lfCamera *LC_lensfun::findCamera(const char *cameraMaker,
                                       const char *cameraModel) const {
  const auto cameras = ldb->FindCamerasExt(cameraMaker, cameraModel);

  if (!cameras) {
#ifdef DEBUG
    std::cerr << "Cannot find the camera \"" << cameraMaker << ": "
              << cameraModel << "\""
...
const lfLens *LC_lensfun::findLens(const lfCamera *camera,
  ...
  for (int i = 0; lenses[i]; ++i) {
    std::cerr << "** lens" << i << " = " << lenses[i]->Maker << ": "
              << lenses[i]->Model << std::endl;
  }
#endif
...
const lfLens *const *LC_lensfun::getLenses() const { return ldb->GetLenses(); }
...
void LC_lensfun::initModifier(int fullWidth, int fullHeight,
...
void LC_lensfun::initModifier(int fullWidth, int fullHeight, float crop,
...
#if (LF_VERSION >= 0x00035f00) // 0.3.95
  mod = std::make_unique<lfModifier>(crop, fullWidth, fullHeight, LF_PF_U16);
#else
  mod = std::make_unique<lfModifier>(lens, crop, fullWidth, fullHeight);
#endif
  if (!mod) {
...
void LC_lensfun::applyModifier(const unsigned short *srcData,
                               unsigned short *dstData, int srcRectX,
                               int srcRectY, int srcRectWidth,
...
#pragma omp parallel
  {
    std::vector<float> pos(dstRectWidth * 2 * 3);

#pragma omp for schedule(guided)
    for (int y = dstRectY; y < dstRectY + dstRectHeight; ++y) {
      mod->ApplySubpixelGeometryDistortion(dstRectX, y, dstRectWidth, 1,
                                           &pos[0]);
      ...
      for (int x = dstRectX, i = 0; x < dstRectX + dstRectWidth; ++x, i += 6) {
        ...
        if (srcRX < srcRectX || srcRX >= srcRectX + srcRectWidth ||
          ...
          dstData[dstIdx + dstROffset] =
              MitchellInterp(srcData, srcPixelStride, srcROffset, srcLineStride,
                             srcRX - srcRectX, srcRY - srcRectY);
          dstData[dstIdx + dstGOffset] =
              MitchellInterp(srcData, srcPixelStride, srcGOffset, srcLineStride,
                             srcGX - srcRectX, srcGY - srcRectY);
          dstData[dstIdx + dstBOffset] =
              MitchellInterp(srcData, srcPixelStride, srcBOffset, srcLineStride,
...
void LC_lensfun::backwardMapRect(int *srcRectParams, int dstRectX, int dstRectY,
                                 int dstRectWidth, int dstRectHeight) const {
  std::vector<float> top(dstRectWidth * 2 * 3);
  std::vector<float> bottom(dstRectWidth * 2 * 3);
  std::vector<float> left(dstRectHeight * 2 * 3);
  std::vector<float> right(dstRectHeight * 2 * 3);

  mod->ApplySubpixelGeometryDistortion(dstRectX, dstRectY, dstRectWidth, 1,
                                       &top[0]);
  mod->ApplySubpixelGeometryDistortion(dstRectX, dstRectY + dstRectHeight,
                                       dstRectWidth, 1, &bottom[0]);
  mod->ApplySubpixelGeometryDistortion(dstRectX, dstRectY, 1, dstRectHeight,
                                       &left[0]);
  mod->ApplySubpixelGeometryDistortion(dstRectX + dstRectWidth, dstRectY, 1,
                                       dstRectHeight, &right[0]);
