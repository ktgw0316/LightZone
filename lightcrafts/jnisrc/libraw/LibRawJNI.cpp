...
JNIEXPORT void JNICALL Java_com_lightcrafts_image_libs_LibRaw_createLibRawObject(JNIEnv *env, jobject obj) {
    env->SetLongField(obj, env->GetFieldID(env->GetObjectClass(obj), "libRawObject", "J"), (long) new LZLibRaw());
}
...
JNIEXPORT jint JNICALL Java_com_lightcrafts_image_libs_LibRaw_openFile(JNIEnv *env, jobject obj, jstring jFileName) {
    jclass libRawClass = env->GetObjectClass(obj);
    auto *libRaw = (LibRaw *) env->GetLongField(obj, env->GetFieldID(libRawClass, "libRawObject", "J"));
    ...
    if (libRaw->imgdata.idata.filters) {
        ...
        for (int i=0; i < 16; i++)
            filter_pattern[i] = libRaw->imgdata.idata.cdesc[libRaw->FC(i >> 1,i & 1)];
        filter_pattern[10] = '\0';
...
extern "C" void interpolate_bayer(void *ptr) {
    ...
    auto data = (uint16_t *) processor->imgdata.image;
    constexpr int pixelStride = 4;
    const int width = processor->imgdata.sizes.width;
    const int height = processor->imgdata.sizes.height;
    const int lineStride = width;

    int rx0, ry0, gx0, gy0, bx0, by0;
    for(int y = 0; y < 2; ++y) {
        for (int x = 0; x + y < 2; ++x) {
            switch(processor->COLOR(y, x)) {
                case 0:
                    rx0 = x;
                    ry0 = y;
                    break;
    ...
    processor->border_interpolate(3);
    std::cout << "PROGRESS 0" << std::endl;
//    RUN_CALLBACK(LIBRAW_PROGRESS_INTERPOLATE, 0, 3);
    interpolateGreen(data, width, height, pixelStride, lineStride, gx0, gy0, ry0);
    std::cout << "PROGRESS 1" << std::endl;
//    RUN_CALLBACK(LIBRAW_PROGRESS_INTERPOLATE, 1, 3);
    interpolateRedBlue(data, width, height, pixelStride, lineStride, rx0, ry0, bx0, by0);
...
JNIEXPORT jshortArray JNICALL Java_com_lightcrafts_image_libs_LibRaw_unpackImage(JNIEnv *env, jobject obj, jboolean interpolate, jboolean half_size) {
    jclass libRawClass = env->GetObjectClass(obj);
    auto *libRaw = (LZLibRaw *) env->GetLongField(obj, env->GetFieldID(libRawClass, "libRawObject", "J"));
    if (libRaw == nullptr)
        return nullptr;
    ...
    libRaw->imgdata.params.output_color = 4; // Kodak ProPhoto RGB D65
    ...
    if (jimage_data != nullptr) {
        env->SetShortArrayRegion(jimage_data, 0, buffer_size, (jshort *) image->data);
    }
    free(image);
    return jimage_data;
}
...
JNIEXPORT jbyteArray JNICALL Java_com_lightcrafts_image_libs_LibRaw_unpackThumb(JNIEnv *env, jobject obj) {
    ...
    if (jthumbnail_data != nullptr) {
        env->SetByteArrayRegion(jthumbnail_data, 0, buffer_size, (jbyte *) thumb->data);
        SET_INT(tformat, thumbnail.tformat);
    }
    free(thumb);
