...
void box_sum_horizontal_and_transpose(const std::vector<float> &src,
                                      std::vector<float> &dst, const int swidth,
                                      const int sheight, const int r) {
  const int dwidth = sheight;
  const int dheight = swidth;

  std::vector<float> srct(sheight * swidth);
...
#pragma omp for nowait
    ...
    for (int y = r + 1, pos0 = (r + 1) * dwidth; y < dheight - r;
         ++y, pos0 += dwidth) {
      OMP_FOR_SIMD
      for (int x = 0; x < dwidth; ++x) {
        const int pos = pos0 + x;
        dst[pos] = dst[pos - dwidth] - srct[pos - (r + 1) * dwidth] +
                   srct[pos + r * dwidth];
      }
    }
  }
...
void box_sum(const std::vector<float> &src, std::vector<float> &dst,
             const int width,
             const int height, // dimensions of the source image
             const int box_radius_x, const int box_radius_y) {
  std::vector<float> buf(height * width);
 ...
 *******************************************************************************/
inline void
nlm_mono_tile(std::vector<float> &ibuf, // pointer to source data buffer
              const float h,            // intensity
              const int sr,             // search window radius in pixels
              const int pr,             // patch radius in pixels
              const float *kernel, // half-kernel containing the exponents of
              ...
              const int height) // dimensions of the source image
{
  ...
  std::vector<float> obuf(elems); // output data buffer
  std::vector<float> w_sum(elems);
  std::vector<float> w_max(elems);
  std::vector<float> v(elems);  // weight map
  std::vector<float> ui(elems); // integral image of square distances

  for (int ny = -sr; ny <= 0; ++ny) {
    for (int nx = -(ny + sr), n_offset = ny * width + nx;
         nx <= ny + sr && n_offset < 0; ++nx, ++n_offset) {
...
inline void nlm_chroma_tile(
    std::vector<float> &ibuf_a, // pointer to the s source/destination buffer
    std::vector<float> &ibuf_b, // pointer to the t source/destination buffer
    const float h,              // intensity
    const int sr,               // search window radius in pixels
    const int pr,               // patch radius in pixels
    const float
    ...
    const int width, const int height) // dimensions of the source image
{
  ...
  const size_t elems = size_t(width) * height;
  std::vector<float> obuf_a(elems); // output data buffer for a
  std::vector<float> obuf_b(elems); // output data buffer for b
  std::vector<float> w_sum(elems);
  std::vector<float> w_max(elems);
  std::vector<float> v(elems);  // weight map
  std::vector<float> ui(elems); // integral image of square distances
  ...
  ibuf_a = std::move(obuf_a);
  ibuf_b = std::move(obuf_b);
}
...
Java_com_lightcrafts_jai_opimage_NonLocalMeansFilterOpImage_nonLocalMeansFilter(
    JNIEnv *env, jclass cls, jshortArray jsrcData, jshortArray jdestData,
    jint y_search_radius, jint y_patch_radius, jint c_search_radius,
    jint c_patch_radius, jfloat y_h, jfloat c_h, jfloatArray jy_kernel,
    jfloatArray jc_kernel, jfloatArray jrgb_to_yst, jfloatArray jyst_to_rgb,
    jint width, jint height, jint srcROffset, jint srcGOffset, jint srcBOffset,
    jint destROffset, jint destGOffset, jint destBOffset, jint srcLineStride,
    jint destLineStride) {
  ...
  const int wr0 = y_search_radius > c_search_radius ? 2 * y_search_radius
                                                    : 2 * c_search_radius;
  ...
  interleaved_RGB_to_planar_YST(srcData, srcLineStride, srcROffset, srcGOffset,
                                srcBOffset, buf_y, buf_s, buf_t, width, height,
                                rgb_to_yst);

  const float *y_kernel_center = y_kernel + y_search_radius;
  const float *c_kernel_center = c_kernel + c_search_radius;
  nlm_mono_tile(buf_y, y_h, y_search_radius, y_patch_radius, y_kernel_center,
                width, height);
  nlm_chroma_tile(buf_s, buf_t, c_h, c_search_radius, c_patch_radius,
                  c_kernel_center, width, height);

  planar_YST_to_interleaved_RGB(destData, destLineStride, destROffset,
                                destGOffset, destBOffset, wr0, buf_y, buf_s,
