...
#define FILTER_MONOCHROME_WRAP_ARGS                                                                \
    JNIEnv *env, jclass cls, jshortArray jsrcData, jshortArray jdestData, jint wr, jint ws,        \
        jfloat scale_r, jfloatArray jkernel, jint width, jint height, jint srcPixelStride,         \
        jint destPixelStride, jint srcOffset, jint destOffset, jint srcLineStride,                 \
        jint destLineStride
...
#ifdef JNI_SSE_BUG
static void filterChromaWrapSSE(FILTER_CHROMA_WRAP_ARGS) __attribute__((used));
#endif /* JNI_SSE_BUG */

JNIEXPORT void JNICALL
Java_com_lightcrafts_jai_opimage_BilateralFilterOpImage_bilateralFilterChromaOld(
    FILTER_CHROMA_WRAP_ARGS) {
#ifdef JNI_SSE_BUG
    JNI_SSE_ASM();

    filterChromaWrapSSE(env, cls, jsrcData, jdestData, wr, ws, scale_r, jkernel, width, height,
...
#ifdef JNI_SSE_BUG
static void filterChromaWrapSSE(FILTER_CHROMA_WRAP_ARGS) {
#endif /* JNI_SSE_BUG */
#if USE_VECTOR
        ...
        while (wlast < row + wr) {
            ...
            if (wlast < height) {
                ...
                for (; col < width - 3; col += 4) {
                    ...
                    _mm_storeu_ps(&windowl[ws - 1][col],
                                  _mm_sqrt_ps(_mm_cvtepi32_ps((__m128i)vr.v) * vnorm));
                ...
                for (; col < width; col++) {
                    float r =
                        srcData[3 * col + wlast * srcLineStride + srcROffset] / (float)0x10000;
        ...
        for (col = 0; col < width; col++) {
            ...
            if (!SEPARABLE) {
                ...
                for (y = row > wr ? -wr : -row; y <= wr && (row + y) < height; y++) {
                    ...
                    if (ws > 3) {
                        ...
                        for (; x <= wr - 3 && (col + x) < width - 3; x += 4) {
                            ...
                            vFloat vexp = vexp_estimate(
                                (SQR(vxy0 - v0) + SQR(vxy1 - v1) + SQR(vxyl - vl)) * vscale_r +
                                vky + loadUnalignedFloat(&kernel[x]));
                            vsa.v += vexp * vxy0;
                            vsb.v += vexp * vxy1;
                            vss.v += vexp;
                        }
                    ...
                    for (; x <= wr && (col + x) < width; x++) {
                        ...
                        float exp = fast_exp(
                            -((SQR(wxy0 - w0) + SQR(wxy1 - w1) + SQR(wxyl - wl)) * scale_r + ky +
                              kernel[x]));
                        if (exp > 0) {
                            sa += exp * wxy0;
                            sb += exp * wxy1;
                ...
                if (ws > 3) {
                    ...
                    for (; x <= wr - 3 && (col + x) < width - 3; x += 4) {
                        ...
                        vFloat vexp = vexp_estimate(
                            (SQR(vx0 - v0) + SQR(vx1 - v1) + SQR(vxl - vl)) * vscale_r + vk0 +
                ...
                for (; x <= wr && (col + x) < width; x++) {
                    ...
                    float exp =
                        fast_exp(-((SQR(wx0 - w0) + SQR(wx1 - w1) + SQR(wxl - wl)) * scale_r + k0 +
                                   kernel[x]));
                    if (exp > 0) {
                        sa += exp * wx0;
                        sb += exp * wx1;
                ...
                if (ws > 3) {
                    ...
                    for (; y <= wr - 3 && (row + y) < height - 3; y += 4) {
                        ...
                        vFloat vexp = vexp_estimate(
                            (SQR(y0.v - v0) + SQR(y1.v - v1) + SQR(yl.v - vl)) * vscale_r + vk0 +
                ...
                for (; y <= wr && (row + y) < height; y++) {
                    ...
                    float exp =
                        fast_exp(-((SQR(wy0 - w0) + SQR(wy1 - w1) + SQR(wyl - wl)) * scale_r +
                                   kernel[y] + k0));
                    if (exp > 0) {
                        sa += exp * wy0;
                        sb += exp * wy1;
            ...
            if (col >= wr && col < width - wr && row >= wr && row < height - wr) {
                ...
                destData[dstPixelOffset + destROffset] =
                    (unsigned short)r; // (r < 0 ? 0 : r > 0xffff ? 0xffff : r);
                destData[dstPixelOffset + destGOffset] =
                    (unsigned short)(g < 0 ? 0 : g > 0xffff ? 0xffff : g);
                destData[dstPixelOffset + destBOffset] =
                    (unsigned short)(b < 0 ? 0 : b > 0xffff ? 0xffff : b);
            }
        }
    }
                ...
                for (y = row > wr ? -wr : -row; y <= wr && (row + y) < height; y++) {
                    ...
                    if (ws > 3) {
                        ...
                        for (; x <= wr - 3 && (col + x) < width - 3; x += 4) {
                            ...
                            vFloat vexp = vexp_estimate(SQR(wxy - v0) * vscale_r + vky +
                                                        loadUnalignedFloat(&kernel[x]));
...
#ifdef JNI_SSE_BUG
static void filterMonochromeWrapSSE(FILTER_MONOCHROME_WRAP_ARGS) __attribute__((used));
#endif /* JNI_SSE_BUG */

JNIEXPORT void JNICALL
Java_com_lightcrafts_jai_opimage_BilateralFilterOpImage_bilateralFilterMonoOld(
    FILTER_MONOCHROME_WRAP_ARGS) {
#ifdef JNI_SSE_BUG
    JNI_SSE_ASM();

    filterMonochromeWrapSSE(env, cls, jsrcData, jdestData, wr, ws, scale_r, jkernel, width, height,
                            srcPixelStride, destPixelStride, srcOffset, destOffset, srcLineStride,
                            destLineStride);
}
#endif /* JNI_SSE_BUG */

#ifdef JNI_SSE_BUG
static void filterMonochromeWrapSSE(FILTER_MONOCHROME_WRAP_ARGS) {
#endif /* JNI_SSE_BUG */
#if USE_VECTOR
    csr_context old_csr = denormals_off();
#endif
    ...
    for (wlast = -1, row = 0; row < height; row++) {
        while (wlast < row + wr) {
            ...
            if (wlast < height) {
                ...
                for (; col < width - 3; col += 4) {
                    vInt vg = vec_unpacklo_epu16((vUInt16)_mm_loadu_si128(
                        (__m128i
                             *)&srcData[srcPixelStride * col + wlast * srcLineStride + srcOffset]));
                    _mm_storeu_ps(&window[ws - 1][col],
                                  _mm_sqrt_ps(_mm_cvtepi32_ps((__m128i)vg) * vnorm));
                }
#endif
                for (; col < width; col++) {
                    float g = srcData[srcPixelStride * col + wlast * srcLineStride + srcOffset] /
                              (float)0x10000;
        ...
        for (col = 0; col < width; col++) {
            ...
            if (col >= wr && col < width - wr && row >= wr && row < height - wr) {
                ...
                destData[dstPixelOffset + destOffset] =
                    (unsigned short)(g1 < 0 ? 0 : g1 > 0xffff ? 0xffff : g1);
            }
        }
    }
    free(window);
